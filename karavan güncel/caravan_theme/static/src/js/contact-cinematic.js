...
(function () {
    ...
    function initMobileSticky() {
        var page = document.querySelector('.ct-hero');
        if (!page) return;
        var wrap = document.getElementById('wrapwrap') || document.body;
        if (wrap.querySelector('.ct-mobile-sticky')) return;
        var sticky = document.createElement('div');
        sticky.className = 'ct-mobile-sticky';
        sticky.innerHTML = '<button class="cv-btn cv-btn-gold" onclick="document.getElementById(\'ct-form-section\').scrollIntoView({behavior:\'smooth\'})"><svg width=18 height=18 viewBox="0 0 24 24" fill=none stroke=currentColor stroke-width=2><line x1=22 y1=2 x2=11 y2=13/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>\u0130leti\u015Fime Ge\u00E7</button>';
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (form) {
            form.addEventListener('submit', function (e) {
                e.preventDefault();
                var success = document.getElementById('ctFormSuccess');
                if (success) success.classList.add('show');
            });
        }
        var newMsgBtn = document.getElementById('ctNewMessage');
        if (newMsgBtn) {
        ...
        var liveBtn = document.getElementById('ctLiveChat');
        if (liveBtn) {
            liveBtn.addEventListener('click', function () {
                this.innerHTML = '<svg width=22 height=22 viewBox="0 0 24 24" fill=none stroke=currentColor stroke-width=2><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg> Ba\u011Flan\u0131yor...';
            });
        }
    });
})();
